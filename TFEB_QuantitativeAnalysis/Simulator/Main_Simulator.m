...
sims_list = {{'FIG4-6B', 'OpenLoop', [0.3 0.7], [0.7 0.3 0]};...
    ...
    {'Test3', 'mTOR_Model', [0 1], [1 0 0 0 0 1 0]};...
    {'FIG4-15A3', 'mTOR_Bort', [0 1], [1 0 0 0 0 1 0]};...
    {'FIG4-15B3', 'mTOR_CHX', [0 1], [1 0 0 0 0 1 0]}};
...
for z = 1:length(sims_list)
    ...
    Mod = sims_list{z}{2};  % MOD := OpenLoop, Feedback, mTOR_Model, mTOR_Bort or mTOR_CHX
    ...
    Ts = 15;
    in = [zeros(3*60/Ts,1);ones(3*60/Ts,1);zeros(3*60/Ts,1);ones(3*60/Ts,1);...
        zeros(3*60/Ts,1);ones((dimEXP-5*3*60)/Ts,1)];
    % ---------------------------------------------------------------------
    % Plots
